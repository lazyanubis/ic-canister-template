...
impl InnerState {
    ...
    fn clean_hash_path(&mut self, hash: HashDigest, path: &str, size: u64) {
    ...
    fn put_file(&mut self, path: String, headers: Vec<(String, String)>, hash: HashDigest, size: u64) {
        ...
        let old_asset = self.files.get(&path).map(|file| (file.hash, file.size));
        ...
        if let Some((old_hash, old_size)) = old_asset
            && old_hash != hash
        {
            self.clean_hash_path(old_hash, &path, old_size);
        }
    ...
    fn assure_upload_capacity(&self, arg: &UploadingArg) {
        ...
        let asset_bytes = self.files.values().try_fold(0_u64, |bytes, file| {
            if hashes.insert(file.hash) {
                bytes.checked_add(file.size)
            } else {
                Some(bytes)
            }
        });
        let uploading_bytes = self
            .uploading
            .values()
            .try_fold(0_u64, |bytes, file| bytes.checked_add(file.size));
        let projected = asset_bytes
            .zip(uploading_bytes)
            .and_then(|(asset_bytes, uploading_bytes)| {
                asset_bytes.checked_add(uploading_bytes.saturating_sub(replacing_size))
            })
